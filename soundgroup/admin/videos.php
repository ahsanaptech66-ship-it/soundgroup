...
<body class="admin-body">
  <div class="admin-app" data-admin-user-id="<?= (int)$user['id'] ?>" data-admin-csrf="<?= htmlspecialchars($csrf, ENT_QUOTES, 'UTF-8') ?>">
    <aside class="admin-sidebar" id="admin-sidebar">
      ...
      <div class="admin-sidebar-bottom">
        <div class="admin-user-mini"><span class="admin-avatar" id="admin-avatar"><?= htmlspecialchars(strtoupper(substr($user['name'], 0, 1)), ENT_QUOTES, 'UTF-8') ?></span>
    ...
    <div class="admin-main-wrap">
      <header class="admin-topbar">
        <div class="admin-topbar-left"><button class="admin-icon-btn admin-mobile-menu" id="admin-sidebar-open" aria-label="Open navigation">☰</button>
          <div><span class="eyebrow">CONTENT CONTROL</span>
      ...
      <main class="admin-main admin-video-main">
        <section class="admin-welcome admin-video-head">
          <div><span class="eyebrow">PHASE 4</span>
            <h1>Shape the video catalog.</h1>
            <p>Upload, edit, publish and curate video without touching the public site structure.</p>
          </div>
        ...
        <section class="admin-video-toolbar glass-card">
          ...
          <select id="video-sort" class="admin-select">
          ...
          </select><select id="video-status" class="admin-select">
            <option value="all">All status</option>
            <option value="published">Published</option>
            <option value="draft">Draft</option>
          </select><select id="video-flag" class="admin-select">
        ...
        <section class="admin-panel glass-card admin-upload-panel" id="upload">
          <div class="admin-panel-head">
            <div><span class="eyebrow">UPLOAD CENTER</span>
              <h2>Add video to SOUNDGROUP</h2>
            </div><button type="button" class="admin-btn admin-btn-ghost" id="close-video-upload">Collapse</button>
          </div>
          <form id="video-upload-form" class="admin-upload-grid" enctype="multipart/form-data">
            <label class="admin-field admin-field-wide"><span>Video files</span>
              ...
              <div class="admin-upload-files" id="video-upload-file-list"><span>No files selected.</span></div>
            </label>
            ...
            <label class="admin-field"><span>Publish on upload</span><select id="upload-published" class="admin-select">
                <option value="1">Published</option>
                <option value="0">Draft</option>
              </select></label>
            <label class="admin-field"><span>Featured</span><select id="upload-featured" class="admin-select">
                <option value="0">No</option>
                <option value="1">Yes</option>
              </select></label>
            <label class="admin-field"><span>Trending</span><select id="upload-trending" class="admin-select">
                <option value="0">No</option>
                <option value="1">Yes</option>
              </select></label>
        ...
        <section class="admin-panel glass-card">
          <div class="admin-panel-head">
            <div><span class="eyebrow">VIDEO LIBRARY</span>
              <h2>Published + draft video</h2>
            </div><span class="admin-panel-meta" id="video-summary">Loading…</span>
          </div>
          <div class="admin-table-wrap">
            <table class="admin-table admin-video-table">
              <thead>
                <tr>
                  <th><input type="checkbox" id="select-all-videos" aria-label="Select all visible videos"></th>
                  <th>Video</th>
                  <th>Artist</th>
                  <th>Genre</th>
                  <th>Status</th>
  ...
  <div class="admin-modal" id="edit-video-modal" aria-hidden="true">
    ...
    <div class="admin-modal-card glass-card" role="dialog" aria-modal="true" aria-labelledby="edit-video-modal-title">
      <div class="admin-panel-head">
        <div><span class="eyebrow">EDIT VIDEO</span>
          <h2 id="edit-video-modal-title">Update video</h2>
        </div><button class="admin-icon-btn" data-close-modal aria-label="Close">×</button>
      </div>
      <form id="edit-video-form" class="admin-edit-grid">
        ...
        <label class="admin-field"><span>Published</span><select id="edit-video-published">
            <option value="1">Published</option>
            <option value="0">Draft</option>
          </select></label>
        <label class="admin-field"><span>Featured</span><select id="edit-video-featured">
            <option value="0">No</option>
            <option value="1">Yes</option>
          </select></label>
        <label class="admin-field"><span>Trending</span><select id="edit-video-trending">
            <option value="0">No</option>
            <option value="1">Yes</option>
          </select></label><label class="admin-field"><span>Artwork</span><input id="edit-video-artwork" type="file" accept="image/jpeg,image/png,image/webp,image/gif"><small class="admin-field-note">Optional replacement. Leave empty to keep current.</small></label>
        ...
        <div class="admin-artwork-remove admin-field-wide"><label><input id="edit-video-remove-artwork" type="checkbox"> Remove current artwork</label></div>
        <div class="admin-upload-actions admin-field-wide"><button type="button" class="admin-btn admin-btn-ghost" data-close-modal>Cancel</button><button type="submit" class="admin-btn admin-btn-primary">Save changes</button></div>
      </form>
    </div>
  </div>
  ...
  <div class="admin-modal" id="video-bulk-edit-modal" aria-hidden="true">
    ...
    <div class="admin-modal-card glass-card admin-bulk-edit-modal-card" role="dialog" aria-modal="true" aria-labelledby="video-bulk-edit-modal-title">
      <div class="admin-panel-head">
        <div><span class="eyebrow">BULK EDIT</span>
          <h2 id="video-bulk-edit-modal-title">Edit selected videos</h2>
          <p class="admin-modal-subtext"><span id="video-bulk-edit-selected-count">0</span> items selected. Only status flags will be changed.</p>
        </div><button class="admin-icon-btn" data-close-video-bulk-edit aria-label="Close">×</button>
      </div>
      <form id="video-bulk-edit-form" class="admin-edit-grid">
        <label class="admin-field"><span>Published</span><select id="video-bulk-edit-published">
            <option value="">No change</option>
            <option value="1">Published</option>
            <option value="0">Draft</option>
          </select></label>
        <label class="admin-field"><span>Featured</span><select id="video-bulk-edit-featured">
            <option value="">No change</option>
            <option value="1">Yes</option>
            <option value="0">No</option>
          </select></label>
        <label class="admin-field"><span>Trending</span><select id="video-bulk-edit-trending">
            <option value="">No change</option>
            <option value="1">Yes</option>
            <option value="0">No</option>
          </select></label>
